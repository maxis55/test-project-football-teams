...
const fetchLeagueStanding = () => {
    axios.get(route('api.leagues.get-league-standing-and-predictions', props.league.id))
        .then(({data}) => {
            leagueResults.value = data.data.standings;
            leagueTeamsPredictions.value = data.data.predictions;
        })
        .catch((error) => {
            console.log(error);
        });
...
const playAll = () => {
    axios.post(route('api.leagues.play-all-remaining-matches', props.league.id))
        .then(({data}) => {
            fetchLeagueStanding();
            fetchLMatchResults();
        })
